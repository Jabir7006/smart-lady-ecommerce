...
import { useUpdateSubCategory, useGetSubCategory } from '../../hooks/useSubCategories';
...
const EditSubCategory = () => {
  ...
  const { data: subCategory, isLoading } = useGetSubCategory(id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'Error updating subcategory', { id: loadingToast });
    } finally {
  ...
  return (
    <div>
      <PageTitle>Edit Sub Category</PageTitle>

      {/* Breadcrumb */}
      <div className="flex text-gray-800 dark:text-gray-300">
        <div className="flex items-center text-purple-600">
          <Icon className="w-5 h-5" aria-hidden="true" icon={HomeIcon} />
          <NavLink to="/app/all-sub-categories" className="mx-2">
            All Sub Categories
          </NavLink>
        </div>
        {">"}
        <p className="mx-2">Edit Sub Category</p>
      </div>
